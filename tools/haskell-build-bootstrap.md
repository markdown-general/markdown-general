...
sed -n '/^```bash$/,/^```$/p' ~/sisyphus/content/tools/haskell-build-bootstrap.md | sed '1d;$d' > /tmp/bootstrap.sh
...
Save and execute this script from ~/sisyphus:

```bash
#!/bin/bash
# Bootstrap script for sisyphus haskell-build system
# Run from ~/sisyphus directory
...
if [ ! -d "content/tools" ]; then
    echo "ERROR: Run this from ~/sisyphus directory"
    exit 1
fi
...
```bash
ls -la ~/sisyphus/artifacts/haskell-build/
cat ~/sisyphus/artifacts/haskell-build/haskell-build.cabal
```
