...
cache_tool() {
  sed -n '/^```python$/,/^```$/p' ~/sisyphus/tools/CARD.md | sed '1d;$d' | python3 - "$@"
}
```
...
```bash
#!/bin/bash
sed -n '/^```python$/,/^```$/p' ~/sisyphus/tools/toolname.md | sed '1d;$d' | python3 - "$@"
```

Make it executable:
```bash
chmod +x ~/sisyphus/artifacts/bin/toolname
```
